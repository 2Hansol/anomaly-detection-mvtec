...
#encoder을 3 channels의 256by256의 이미지를 convolutionnal layer를 통해 100x1x1의 image features를 뽑아낸다.
...
#Decoder는 100x1x1의 image features를 가지고 원래의 이미지 크기로 generate를 한다. 
class Decoder(nn.Module):
    def __init__(self, img_shape, latent_dim):
        super().__init__()
        self.img_shape = img_shape
        self.model = nn.Sequential(
            # Transpose conv--> deconvolution이라고도 한다. 차원을 늘려주게 된다. 
